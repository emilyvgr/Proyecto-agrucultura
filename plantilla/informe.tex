...
\emph{9,884} observaciones (registros) donde cada fila representa un
...
(Estos registros son previos al ETL)
...
mayor entre los años 2017-2019 (previo a al ETL), El procesamiento de la
información se realizó con corte a febrero de 2026.

\section{Variables}

La variable principal en estudio (Variable Dependiente/Objetivo) es
Season, que representan las Temporadas de cultivo, medido en 3
estaciones ``Kharif'' (Otoño), ``Zaid'' (Verano corto) y ''Rabi''
(Invierno) ,siendo las principales temporadas de cultivo en la India
...
estadísticos necesarios para el análisis de los datos de influencia en
ciclos agrícolas. Se aborda la teoría desde una perspectiva descriptiva,
priorizando la caracterización de la distribución de los ciclos
agrícolas (variable independiente) y la identificación de patrones a lo
largo del tiempo.

La comprensión de estos conceptos es crucial para interpretar
correctamente las variables influyentes como el Annual\_Rainfall ,
Fertilizer y Pesticide . Las cuales se compararan con las variables
explicativas Area , Yield y Production. para dar resultados y
...
datos, a menudo con métodos visuales, sin formular hipótesis a priori.\\
En el contexto de este trabajo, el EDA permitirá detectar
\emph{outliers} nulos y mal categorizados (estaciones repetidas) y
verificar si la distribución de las variables influyentes sigue una
distribución parecida a la de la variable independiente (Season).\\

\subsection{Cocientes estadísticos:}

Los cocientes estadísticos son la razón, la proporción, el porcentaje,
las tasas entre otros. El cociente me dice que cantidad del denominador
este contenido en el numerador, Fueron desarrollados por Gaspar Neumann
y Edmund Halley. Es importante para nuestro estudio porque tomaremos
conceptos como la razón, tasas para comparar las relaciones de nuestras
variables influyentes en los ciclos agrícolas.\\

\textbf{Razón}: Es cuando existe una relación cuantitativa entre 2
cantidades que comparten el mismo total. Fue Karl Pearson, Francis
Galton pioneros en el desarrollo de este concepto. Tomaremos este
concepto importante para comprender la relación cuantitativa entre las
variables \emph{Pesticide} y \emph{Fertilizer} en las temporadas de
cultivos.

\textbf{Tasas}: Son razones dinámicas porque transcurren en el tiempo y
se multiplican generalmente por 1000. John Graunt y figuras posteriores
como Bayes o Bernoulli la continuaron desarrollándola. La usaremos para
saber la cantidad de agroquímicos en un determinado año.

\subsection{Medidas de Posición:}

Son medidas que ayudan a ubicar los datos fácilmente. Están constituidas
principalmente por cuartiles, deciles y percentiles. Estos dividen el
conjunto de datos en distintas partes, dividiendo los cuartiles en 4
partes iguales, los de deciles en partes iguales de 10 y los percentiles
dividiendo en 100 partes iguales. Para nuestro informe se usará para
obtener indicadores importantes que nos ayudaran a analizar las
distribuciones de los datos apoyando a responder preguntas de
investigación y objetivos planteados. También este concepto se usa en el
Análisis Exploratorio de Datos (EDA). Thomas Bayes, Jakob Bernoulli,
Pafnuti Chebyshev y Francis Galton fueron los pioneros desarrollando
este tema.\\

\subsection{Medidas Tendencia Central:}

Son medidas ubicadas en el centro de la distribución. Denominados
promedios y se dividen en dos tipos los matemáticos y los no
matemáticos. Enfocando este tema al estudio es de importancia ya que
estas medidas nos permiten comprender las tendencias de equilibrio a
esperar en los datos. Los autores principales que desarrollaron este
tema fueron: Gauss, Karl Pearson y Cournot.\\

\textbf{Media Aritmética}: Es la representación de los datos manteniendo
un equilibrio en toda la serie de datos, esta definición es
importantísima para la estadística ya que es un valor que agrupa un
conjunto de datos, en el contexto de nuestro estudio es fundamental para
responder a lo planteado. Carl Friedrich Gauss\,y\,Adrien-Marie Legendre
desarrolladores de esta importante medida.

\textbf{Mediana}: Es el valor que ocupa la posición central de un
conjunto de datos ordenado. Centrado en nuestro estudio es relevante ya
que nos permite obtener un indicador más robusto a valores atípicos es
decir una medida más representativa. Antoine Augustin Cournot y Francis
Galton fueron pioneros en esta definición.

\textbf{Moda}: Es el valor que aparece con mayor frecuencia en el
conjunto de datos. Orientado en el informe es de interés ya que nos
dictara la mayor frecuencia de los datos, indicando que ese dato es
importante ya que dirá una representación de lo común. Karl Pearson fue
el principal desarrollador de la moda donde se basó en una idea de que
siempre destacaba un dato ante los demás por ser el más común.

\textbf{Media Geométrica}:\,La media geométrica es igual a la raíz
n-esima del producto de n valores positivos, es decir multiplica los
valores a diferencia de la media aritmética, lo que es ideal cuando se
desea saber el promedio del tanto por ciento o disminución de una cosa.
Desde épocas de Pitágoras hasta las bases firmes de Irving Fisher esta
medida ha llevado contribuciones a lo largo del tiempo. Llegando a ser
una alternativa excelente cuando la media no es lo indicado.

\textbf{Media Armónica}: Se denota H y se obtiene como los recíprocos de
una serie dada, luego se determina la media aritmética de dichos
recíprocos, y se toma la inversa de dicha media como la media armónica.
Situando este concepto en el análisis de la influencia a los ciclos
agrícolas es válido ya que promedia cantidades de químicos aplicados en
una hectárea en específico. Desde épocas de Archytas de Tarento hasta
Isaac Newton y Gottfried Leibniz se desarrolló inicialmente este
concepto, basándose un poco en la música y en la física.

\subsection{Medidas de dispersión: }

Son medidas que buscan saber la variabilidad con respecto los valores
centrales, dividiéndose en absolutas y relativas. Considerándola en la
investigación presenta una notaría importancia ya que la influencia
depende mucho de la dispersión de los factores. Abraham de Moivre Carl
Friedrich Gauss , Francis Galton , Karl Pearson Ronald A. Fisher
desarrolladores de estas medidas.\\

\textbf{Rango}: Es el recorrido del conjunto de valores, expresado como
la diferencia entre el Valor Máximo y el Valor Mínimo de la serie.
Representando la mayor diferencia en los datos, pertenece a las familias
de las absolutas. Implementada al informe es crucial para entender la
diferencia entre los datos. Es una de las medidas más antiguos ya que es
intuitiva y fácil de entender por ello se ha aplicado desde casi
siempre. Uno de los primeros fue Hiparco de Nicea.

\textbf{Rango intercuartílico}: Es igual a la diferencia entre el tercer
cuartil y el primer cuartil. Lo que quiere decir que nos queda el 50 \%
central de los datos. siendo fundamental para la obtención de resultados
robusto no propensos a valores atípicos. Desarrollado por Francis Galton
un principal autor del concepto.

\textbf{Desviación Media}: Es el promedio de las distancias entre los
valores de la serie y la medida de tendencia central escogida.
Interesante para esperar una dispersión entre los datos a estudiar.
Roger Cotes estudio esta medida para que comenzara ha definirse.

\textbf{Varianza (Volatilidad)}: Mide la dispersión de las
precipitaciones respecto a la media, fundamental para medir el riesgo de
precipitación en un año. Ronald A. Fisher formalmente introdujo el tema
de la varianza.

\textbf{Desviación estándar}: Es la raíz cuadrada positiva de la
varianza. Constituye el índice de dispersión más usado y confiable.
Resaltando la dispersión entre las variables influyentes de nuestro
estudio. Karl Pearson para muchos el más influyente en las medidas de
dispersión.

\textbf{Coeficiente de variación}: Pertenece a las medidas relativas,
siendo así una medida que nos dice el porcentaje de deviación, existe
varios coeficientes de variación, pero el más famoso es el de Pearson.
Fundamental para comparar medias e indicadores de interés para el
informe. Karl Pearson sin duda unos de los principales autores en las
medidas relativas.

\textbf{Regla Empírica}: usado de forma gráfica para ver el
comportamiento y desviaciones de los datos con respecto a la
distribución. Aplicado en el estudio para la identificación de valores
atípicos. Abraham de Moivre\,y\,Carl Friedrich Gauss.

\subsection{Medidas de Forma:}

Son indicadores que nos dicen como se distribuyen los datos, teniendo
una ventaja de que no necesariamente hay que graficar para observar las
distribuciones. Luego de comprender los que es la simetría y la
asimetría estos indicadores son importantes ya que nos permiten ver si
hay alguna existencia de sesgo en los datos. William Gosset, Francis
Galton Ronald A. Fisher, desarrolladores de estas medidas.\\

\textbf{Asimetría}: Es cuando los datos se presentan en una distribución
desproporcionada a cualquier lado de la media aritmética. Las medidas de
asimetría son indicadores que nos dicen si la cola de la distribución es
sesgada a la izquierda (negativa) o a la derecha (positiva), llevándolo
a el análisis de influencia en las temporadas de cultivos, permitirá
observar la tendencia de los datos sirviendo de sustentos para las
respuestas. Pearson Galton y Fisher precursores de este tema.

\textbf{Kurtosis}: Medida estadística utilizada para determinar cuán
puntiaguda es la distribución de una serie de datos tomando como
referencia la curva normal. Clasificándose de la siguiente manera:
Leptocúrtica, Mesocúrtica y Platicúrtica, evidenciando una concentración
de los datos alrededor del valor máximo de la curva. Existiendo
distintos coeficientes para acertar mejor al objetivo plateado. Karl
Pearson principal desarrollador de la definición.
